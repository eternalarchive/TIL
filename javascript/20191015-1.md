...
- [[Get]]을 읽어보면 : 프로퍼티 키 유효 확인 -> (접근자/데이터 프로퍼티 확인해보고) 데이터 프로퍼티라면 -> [[Value]]에 할당되어진 값을 반환해라 / 접근자 프로퍼티라면 [[Get]] 값 반환
- [[Set]]을 읽어보면 : 프로퍼티 키 유효 확인 -> (접근자/데이터 프로퍼티 확인해보고) 데이터 프로퍼티라면 -> 객체 외부 슬롯의 값을 재할당 / 접근자 프로퍼티라면 [[Set]]을 호출
